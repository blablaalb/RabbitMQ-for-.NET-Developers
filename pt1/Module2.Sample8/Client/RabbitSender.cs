...
public class RabbitSender : IDisposable
{
    ...
    private async Task SetupRabbitMq()
    {
        ...
        var responseQueue = await _model.QueueDeclareAsync();
        _responseQueue = responseQueue.QueueName;
        _consumer = new AsyncEventingBasicConsumer(_model);
        await _model.BasicConsumeAsync(_responseQueue, true, _consumer);
    }

    public async Task SendAsync(string message, string routingKey, Action<IList<string>> callback)
    {
        ...
        //Send
        await _model.BasicPublishAsync(ExchangeName, routingKey, mandatory: true, properties, messageBuffer);
        _consumer.ReceivedAsync += async (ch, ea) =>
        {
            if (ea.BasicProperties.CorrelationId == correlationToken)
            {
    ...
    public void Dispose()
    {
        ...
        if (_model != null && _model.IsOpen)
            _model.AbortAsync().GetAwaiter().GetResult();
